...
class MultiheadAttention(nn.Module):
    def __init__(
        ...
        self.in_proj_weight = nn.Parameter(
            torch.ones((3 * self.all_head_size, self.all_head_size)), requires_grad=True
        )
        self.in_proj_bias = nn.Parameter(
            torch.zeros((3 * self.all_head_size)), requires_grad=True
        )
    ...
    def transpose_for_scores(self, x):
        x = x.contiguous().view(
            -1, x.size(1) * self.num_attention_heads, self.attention_head_size
        )
    ...
    def forward(self, hidden_states):
        ...
        mixed_layer = (
            torch.matmul(hidden_states, self.in_proj_weight.t()) + self.in_proj_bias
        )
        mixed_query_layer, mixed_key_layer, mixed_value_layer = torch.chunk(
            mixed_layer, 3, -1
        )
        ...
        if self.num_attention_heads == 8:
            ...
            attention_mask = torch.tril(
                torch.ones(1, hidden_states.size(0), hidden_states.size(0)), diagonal=0
            )
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states_to_ln = self.ln_1(hidden_states.type(torch.float32)).type(
            hidden_states.dtype
        )
        ...
        hidden_states_to_ln = self.ln_2(hidden_states.type(torch.float32)).type(
            hidden_states.dtype
        )
...
class VisualTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            torch.ones((patch_number + 1, hidden_size)), requires_grad=True
        )
        ...
        self.proj = nn.Parameter(
            torch.ones((hidden_size, proj_size)), requires_grad=True
        )
    ...
    def forward(self, input):
        ...
        # bs*49*768
        vis_emb = input.reshape(-1, self.hidden_size, self.patch_number).permute(
            0, 2, 1
        )
        # bs*1*768
        cls_emb = (
            self.class_embedding.unsqueeze(0).unsqueeze(1).repeat(vis_emb.size(0), 1, 1)
        )
        # bs*50*768
        ...
        input_emb = (
            self.ln_pre(input_emb.type(torch.float32))
            .type(input_emb.dtype)
            .permute(1, 0, 2)
        )
        # bs*50*768
...
class CLIP(nn.Module):
    def __init__(
        ...
        self.token_embedding = nn.Embedding(
            int(self.vocab_size.item()), text_hidden_size
        )
        self.ln_final = nn.LayerNorm(text_hidden_size, eps=1e-5)
        self.positional_embedding = nn.Parameter(
            torch.ones((int(self.context_length.item()), text_hidden_size)),
            requires_grad=True,
        )
        self.text_projection = nn.Parameter(
            torch.ones((text_hidden_size, text_hidden_size)), requires_grad=True
        )
